...
This repository is designed to keep source-controlled assets separate from local operational evidence and secrets.
...
Examples that should remain local for this repository:
...
Use example environment files (for example `.env.example`) for documentation only.
